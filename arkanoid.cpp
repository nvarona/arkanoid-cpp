...
int auxPastilla;
int cantBloques = 56;        // Cantidad de Bloques Activos
CircleShape bola;            // Bola
Vector2f velDirBola(-3, -3); // Direccion y Velocidad Bola
RectangleShape paleta;       // Paleta
bool enColision = false;     // Detectar Colision
bool activo = true;          // Estado del Juego
Font fuente;                 // Fuente y Texto
Text texto;
int idCV = 1, idCH = 1;                  // Auxiliares para Colores
SoundBuffer paredSB, paletaSB, bloqueSB; // Sonidos
Sound paredS, paletaS, bloqueS;
int timer = 0; // Timer/Contador

int main() {
  RenderWindow ventana(VideoMode(650, 500), "ArKanoid"); // Ventana
  ventana.setFramerateLimit(60);
  bola = CircleShape(5.f); // Bola
  ...
  if (!paredSB.loadFromFile("Sounds/ReboteParedes.wav") ||
      ...
      !bloqueSB.loadFromFile("Sounds/ReboteLadrillo.wav")) {
  ...
  while (ventana.isOpen()) {
    ...
    if (activo) { // Si esta activo el juego ejecutamos el codigo
      // Rebote Bola
      if (bola.getPosition().x >= ventana.getSize().x ||
          bola.getPosition().x <= 0) {
      ...
      if (Keyboard::isKeyPressed(Keyboard::Right) &&
          paleta.getPosition().x <= ventana.getSize().x) {
        paleta.move(3.5, 0);
      }
      if (Keyboard::isKeyPressed(Keyboard::Left) &&
          paleta.getPosition().x >= 0) {
        paleta.move(-3.5, 0);
      }
      // Movimiento Bola
      bola.move(velDirBola.x, velDirBola.y);
      // Colision Pastilla
      if (pastilla.estado && paleta.getGlobalBounds().contains(
                                 pastilla.pastilla.getPosition().x,
                                 pastilla.pastilla.getPosition().y)) {
        paleta.setScale(2, 1);
      ...
      if (!enColision) {
        ...
        if (paleta.getGlobalBounds().contains(bola.getPosition().x,
                                              bola.getPosition().y)) {
          if (paleta.getPosition().x - bola.getPosition().x >=
              20) { // Rebote Lado Izquierdo
            velDirBola.x = -3;
          } else if (paleta.getPosition().x - bola.getPosition().x <=
                     -20) { // Rebote Lado Derecho
            velDirBola.x = 3;
        ...
        for (int i = 1; i < 8; i++) {
          for (int j = 1; j < 9; j++) {
            if (bloques[i][j].estado &&
                bloques[i][j].bloque.getGlobalBounds().contains(
                    bola.getPosition().x, bola.getPosition().y)) {
              velDirBola.y *= -1;
              ...
              if (auxPastilla == 1 && !pastilla.estado) {
                ...
                pastilla.pastilla.setPosition(
                    bloques[i][j].bloque.getPosition().x,
                    bloques[i][j].bloque.getPosition().y);
              }
            }
          }
        }
